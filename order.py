    ...
    while True:
        ...
        for category, items in menu.items():
            ...
            for item in items:
                ...
                all_items[formatted_name] = item  # Store by formatted name
                all_items[item['name'].upper().replace(" ", "-")] = item  # Store uppercase names
                all_items[item['name'].title().replace(" ", "-")] = item  # Store title case names
                item_number += 1
        ...
        selected_items = [choice.strip().replace(" ", "-") for choice in item_choices.split(",")]
